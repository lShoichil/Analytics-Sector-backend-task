...
### Валидация полей
При получении невалидных полей выводется лог ошибки 422, где будет написан regex, которому нужно соответсвовать (В принципе логи FastAPI довольно информативны при ошибке 422, поэтому я не делал отдельные исключения с сообщения при проверку каждого поля)
